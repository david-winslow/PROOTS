...
namespace OTS.Web.Helpers
{
  public interface IEmailer
  {
    ...
    Task<(bool success, string errorMsg)> SendEmailAsync(string recepientName, string recepientEmail, string subject,
      string body, SmtpConfig config = null, bool isHtml = true);

    Task<(bool success, string errorMsg)> SendEmailAsync(string senderName, string senderEmail, string recepientName,
      string recepientEmail, string subject, string body, SmtpConfig config = null, bool isHtml = true);
  }
  ...
  public class Emailer : IEmailer
  {
    ...
    public async Task<(bool success, string errorMsg)> SendEmailAsync(MailboxAddress sender,
      MailboxAddress[] recepients, string subject, string body, SmtpConfig config = null, bool isHtml = true)
    {
      ...
      {
        Utilities.CreateLogger<Emailer>()
          .LogError(LoggingEvents.SEND_EMAIL, ex, "An error occurred whilst sending email");
