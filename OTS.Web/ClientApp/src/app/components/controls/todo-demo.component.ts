...
export class TodoDemoComponent implements OnInit, AfterViewInit, OnDestroy {
                        ...
                        data = [
                            {
                                ...
                                description:
                                    "Create a visual studio VSIX extension package that will add this project as an aspnet-core project template"
                            },
                            {
                                isComplete: false,
                                isImportant: true,
                            ...
                            },
                            {
                                isComplete: false,
                                isImportant: false,
                                name: "Create aspnet-core/angular5 tutorials based on this project",
                                description:
                                    "Create tutorials (blog/video/youtube) on how to build applications (full stack)" +
    ...
    saveToDisk() {
        if (this.isDataLoaded) {
            this.localStorage.saveSyncedSessionData(this.dataSource.data,
                `${TodoDemoComponent.DBKeyTodoDemo}:${this.currentUserId}`);
        }
    }

    private filterData(task: ToDoTask, filter: string): boolean {
        return !(task.isComplete && this.hideCompletedTasks) &&
            Utilities.searchArray(filter, false, task.name, task.description);
    }
}
